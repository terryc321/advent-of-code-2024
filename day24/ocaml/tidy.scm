...
(define (ocaml-list-of xs)
  (cond
   ((null? xs) "")
   ((null? (cdr xs)) (format #f "~a" (car xs)))
   (#t (string-append (format #f "~a" (car xs)) ";" (ocaml-list-of (cdr xs))))))

(define (ocaml-reflist-of xs)
  (cond
   ((null? xs) "")
   ((null? (cdr xs)) (format #f "!~a" (car xs)))
   (#t (string-append (format #f "!~a" (car xs)) ";" (ocaml-reflist-of (cdr xs))))))
...
(define (test xs)
  (letrec ((foo (lambda (xs)
		  (cond
		   ((null? xs) xs)
		   (#t (map (lambda (x) (fmt #t "let ~a : int option ref = ref None;;~%" x)) xs))))))
    (let-values (((ass ops syms) (fixup xs)))
      (let* ((tidy-syms (sorted-symbols syms))
	     (tidy-zsyms (filter (lambda (x) (char=? #\z (string-ref (symbol->string x) 0))) tidy-syms)))
	...
	(map (lambda (x) (map (lambda (y) (format #t "~a" y)) x) (format #t "~%")) ass)
	
	;; procedural body here 
	(format #t "~%(* operations  *)~%")
	(format #t "let rec foo () = ~%")
	(format #t "let zs = [~a] in ~%" (ocaml-list-of tidy-zsyms))
	(format #t "let undet = List.filter (fun x -> match (!x) with | Some _ -> false | _ -> true) zs in ~%")
	(format #t "if undet = [] then zs ~%else (")
...
(define (fixup xs)
  (let ((symbols '()))
    (letrec ((add-symbol! (lambda (x)
	     ...
	     (fix-assigns (lambda (x)
			    (let ((sym (symbol-decapitalize (second x)))
				  (val (second (fourth x))))
			      (add-symbol! sym)
			      (list sym " := Some " val ";;"))))
	     (fix-generic (lambda (tag x) 	     
		       (let ((sym1 (symbol-decapitalize (second x)))
			     (sym2 (symbol-decapitalize (fourth x)))
			     (sym3 (symbol-decapitalize (sixth x))))
			 (add-symbol! sym1)
			 (add-symbol! sym2)
			 (add-symbol! sym3)			 
  ...
  (let* ((s1 (apply append (map cdr (cdr xs))))
	 (s2 (map foo (filter (lambda (x) (eq? (car x) 'assigns)) s1)))
	 (s3 (map foo (filter (lambda (x) (not (eq? (car x) 'assigns))) s1))))
    (values s2 s3 symbols)))))
...
(define (run)
 (with-output-to-file "input.ml" (lambda () (test input)))
  (with-output-to-file "example1.ml" (lambda () (test example1)))
  (with-output-to-file "example2.ml" (lambda () (test example2)))
  )
